...
describe("applyCurrentDailyPhotoFileUri", () => {
  it("repairs local file URIs when the app container path changed", () => {
    const photo = createDailyPhoto({
      imagePath:
        "file:///old-container/Documents/daily-photos/local-user/2026-06-30.jpg",
      localImagePath:
        "file:///old-container/Documents/daily-photos/local-user/2026-06-30.jpg",
    ...
    expect(
      applyCurrentDailyPhotoFileUri(
        photo,
        "file:///current-container/Documents/daily-photos/local-user/2026-06-30.jpg",
      ),
    ).toMatchObject({
      imagePath:
        "file:///current-container/Documents/daily-photos/local-user/2026-06-30.jpg",
  ...
  it("keeps photos without a local storage key unchanged", () => {
    ...
    expect(
      applyCurrentDailyPhotoFileUri(
        photo,
        "file:///current-container/photo.jpg",
      ),
    ).toBe(photo);
  });
});
