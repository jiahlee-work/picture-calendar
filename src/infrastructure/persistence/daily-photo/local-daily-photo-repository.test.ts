...
import type { DailyPhoto, DailyPhotoMetadataStore } from "@/shared/daily-photo/types";
...
describe("createLocalDailyPhotoRepository", () => {
  ...
  it("persists metadata through the configured store", async () => {
    ...
    const restoredRepository = createLocalDailyPhotoRepository({ metadataStore });

    await expect(restoredRepository.listByMonth("user-1", "2026-04")).resolves.toMatchObject([
      {
        userId: "user-1",
        date: "2026-04-09",
        imagePath: "file://daily-photos/user-1/2026-04-09.jpg",
  ...
  it("stores daily photos by user and date", async () => {
    ...
    await expect(repository.listByMonth("user-1", "2026-04")).resolves.toMatchObject([
      {
        userId: "user-1",
        imagePath: "file://user-1.jpg",
      },
    ]);
    await expect(repository.listByMonth("user-2", "2026-04")).resolves.toMatchObject([
      {
        userId: "user-2",
        imagePath: "file://user-2.jpg",
      },
