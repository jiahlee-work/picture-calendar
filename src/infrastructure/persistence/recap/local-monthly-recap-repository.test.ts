...
import type { MonthlyRecap, MonthlyRecapMetadataStore } from "@/shared/recap/types";

describe("createLocalMonthlyRecapRepository", () => {
  it("stores at most 10 unique selected photo ids", async () => {
  ...
  it("lists recaps by user and year in month order", async () => {
    ...
    await expect(repository.listByYear("user-1", "2026")).resolves.toMatchObject([
  ...
  it("skips selection and clears selected photos", async () => {
    ...
    const restoredRepository = createLocalMonthlyRecapRepository({ metadataStore });
...
function createFakeMetadataStore(initialRecaps: MonthlyRecap[] = []): MonthlyRecapMetadataStore {
