...
describe("monthly recap photo selection", () => {
  it("adds and removes a selected photo id", () => {
    expect(toggleMonthlyRecapSelectedPhotoId(["photo-1"], "photo-2")).toEqual(["photo-1", "photo-2"]);
    expect(toggleMonthlyRecapSelectedPhotoId(["photo-1", "photo-2"], "photo-1")).toEqual(["photo-2"]);
  });

  it("does not add more than the selection limit", () => {
    const selectedPhotoIds = Array.from({ length: 10 }, (_, index) => `photo-${index + 1}`);

    expect(toggleMonthlyRecapSelectedPhotoId(selectedPhotoIds, "photo-11")).toBe(selectedPhotoIds);
  });
