...
describe("resolveMonthlyRecapTemplatePhotos", () => {
  it.each([4, 5, 6, 7, 8, 9])(
    ...
    (photoCount) => {
      ...
      const resolvedPhotos = resolveMonthlyRecapTemplatePhotos({ photos, recap });

      expect(resolvedPhotos.backgroundPhotos.map((photo) => photo.id)).toEqual([backgroundPhotoId]);
      expect(resolvedPhotos.calendarPhotos.map((photo) => photo.id)).toEqual(calendarPhotoIds);
      expect([
  ...
  it("uses the stored layout instead of choosing a new background", () => {
    ...
    const firstResolvedPhotos = resolveMonthlyRecapTemplatePhotos({ photos, recap });
    const secondResolvedPhotos = resolveMonthlyRecapTemplatePhotos({ photos, recap });

    expect(firstResolvedPhotos.backgroundPhotos.map((photo) => photo.id)).toEqual(["photo-4"]);
    expect(secondResolvedPhotos.backgroundPhotos.map((photo) => photo.id)).toEqual(["photo-4"]);
  });
});
