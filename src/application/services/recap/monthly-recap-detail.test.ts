...
describe("loadMonthlyRecapDetail", () => {
  it("keeps the first generated automatic layout when the same month is opened again", async () => {
    const dailyPhotoRepository = createFakeDailyPhotoRepository(createPhotos(recapTimeline.previousMonth, 6));
    ...
    expect(secondResult.recap?.backgroundPhotoIds).toEqual(firstResult.recap?.backgroundPhotoIds);
    expect(secondResult.recap?.calendarPhotoIds).toEqual(firstResult.recap?.calendarPhotoIds);
  ...
  it("does not create a recap for the current month in production", async () => {
    const dailyPhotoRepository = createFakeDailyPhotoRepository(createPhotos(recapTimeline.currentMonth, 6));
    const recapRepository = createLocalMonthlyRecapRepository();
  ...
  it("does not create a recap for a future month even in development", async () => {
    const dailyPhotoRepository = createFakeDailyPhotoRepository(createPhotos(recapTimeline.futureMonth, 6));
    const recapRepository = createLocalMonthlyRecapRepository();
  ...
  it("refreshes an automatic layout only when the monthly photos change", async () => {
    const dailyPhotoRepository = createMutableDailyPhotoRepository(createPhotos(recapTimeline.previousMonth, 4));
    const recapRepository = createLocalMonthlyRecapRepository();
    const firstResult = await loadMonthlyRecapDetail({
      currentDate: recapTimeline.currentDate,
      dailyPhotoRepository,
    ...
    expect(secondResult.recap?.backgroundPhotoIds).not.toEqual(firstResult.recap?.backgroundPhotoIds);
  });

  it("refreshes a manual layout when a selected representative photo is deleted", async () => {
    const dailyPhotoRepository = createMutableDailyPhotoRepository(createPhotos(recapTimeline.previousMonth, 11));
    const recapRepository = createLocalMonthlyRecapRepository();
    const firstResult = await loadMonthlyRecapDetail({
      currentDate: recapTimeline.currentDate,
      dailyPhotoRepository,
    ...
    await recapRepository.saveSelection({
      ...
      backgroundPhotoIds: ["photo-5", "photo-6", "photo-7", "photo-8", "photo-9", "photo-10"],
    });
    dailyPhotoRepository.setPhotos(createPhotos(recapTimeline.previousMonth, 11).filter((photo) => photo.id !== "photo-10"));
...
function createFakeDailyPhotoRepository(photos: DailyPhoto[]): DailyPhotoRepository {
...
function createMutableDailyPhotoRepository(initialPhotos: DailyPhoto[]) {
  ...
  } satisfies DailyPhotoRepository & { setPhotos: (nextPhotos: DailyPhoto[]) => void };
}
