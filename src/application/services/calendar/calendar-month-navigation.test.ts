...
describe("calendar month navigation", () => {
  ...
  it("prevents previous navigation before January 2026", () => {
    const previousMonth = addNavigableCalendarMonths(dayjs("2026-01-01").toDate(), -1);
  ...
  it("allows January 2026 and later months", () => {
    ...
    expect(canNavigateToCalendarMonth(dayjs("2025-12-01").toDate())).toBe(false);
  });

  it("creates picker years from 2026 through future years", () => {
    const years = createCalendarYearOptions(dayjs("2026-01-01").toDate(), dayjs("2026-06-29").toDate(), 2);
