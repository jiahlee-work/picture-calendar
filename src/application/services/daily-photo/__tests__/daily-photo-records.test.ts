...
describe("daily photo records", () => {
  it("indexes photos by date", () => {
    const firstPhoto = createDailyPhoto({
      date: "2026-06-01",
      imagePath: "file://1.jpg",
    });
    const secondPhoto = createDailyPhoto({
      date: "2026-06-02",
      imagePath: "file://2.jpg",
    });
  ...
  it("detects photos that can be displayed", () => {
    expect(
      isDisplayableDailyPhoto(
        createDailyPhoto({ imagePath: "file://photo.jpg" }),
      ),
    ).toBe(true);
    expect(isDisplayableDailyPhoto(createDailyPhoto({ imagePath: "" }))).toBe(
      false,
    );
  ...
  it("detects legacy development photos persisted from old app builds", () => {
    expect(
      isLegacyDevelopmentDailyPhoto(
        createDailyPhoto({
          imagePath:
            "file:///data/user/0/com.jiahleework.pical/files/PicalSamples/10.jpg",
    ...
    expect(
      isLegacyDevelopmentDailyPhoto(
        createDailyPhoto({
          imagePath:
            "file:///data/user/0/com.jiahleework.pical/files/daily-photos/local-user/2026-06-28.jpg",
