...
vi.mock("@/application/services/daily-photo/daily-photo-repository-factory", () => ({
  createDailyPhotoRepositoryForRuntime: vi.fn(),
}));
vi.mock("@/application/services/notifications/local-notification-adapter-factory", () => ({
  createLocalNotificationAdapterForRuntime: vi.fn(),
}));
vi.mock("@/application/services/recap/monthly-recap-repository-factory", () => ({
  createMonthlyRecapRepositoryForRuntime: vi.fn(),
}));
vi.mock("@/application/services/settings/recap-notification-settings-repository-factory", () => ({
  createRecapNotificationSettingsRepositoryForRuntime: vi.fn(),
}));
vi.mock("@/infrastructure/device/notifications/expo-local-notification-adapter", () => ({
  MONTHLY_RECAP_NOTIFICATION_CHANNEL_ID: "monthly-recap",
}));

import { syncMonthlyRecapNotificationSchedule } from "@/application/services/notifications/recap-notification-service";
import { MonthlyRecapSelectionStatus, MonthlyRecapTemplateId } from "@/application/services/recap/types";
import { dayjs } from "@/shared/date/dayjs";
import type { DailyPhoto, DailyPhotoRepository } from "@/shared/daily-photo/types";
import {
...
import type { MonthlyRecap, MonthlyRecapRepository } from "@/shared/recap/types";
...
describe("monthly recap notification service", () => {
  it("syncs only the next monthly notification window and does not reuse the previous-month preview plan", async () => {
    ...
    expect(scheduledRequests[0]?.triggerDate).toEqual(dayjs("2026-08-01T09:00:00").toDate());
  });
});
...
function createNotificationAdapter(): LocalNotificationAdapter {
  return {
    ...
    getPermissionStatus: vi.fn(async () => LocalNotificationPermissionStatus.granted),
    requestPermission: vi.fn(async () => LocalNotificationPermissionStatus.granted),
    scheduleNotification: vi.fn(async (request) => request.identifier),
  };
}

function createSettingsRepository(isEnabled: boolean): RecapNotificationSettingsRepository {
  return {
...
function createDailyPhotoRepository(photosByMonth: Record<string, DailyPhoto[]>): DailyPhotoRepository {
  return {
    deleteByDate: vi.fn(async () => null),
    hasAny: vi.fn(async () => Object.values(photosByMonth).some((photos) => photos.length > 0)),
...
function createMonthlyRecapRepository(
  ...
  return {
    ...
    listByYear: vi.fn(async () =>
      Object.values(recapsByMonth).filter((recap): recap is MonthlyRecap => recap !== null)),
    markPrompted: vi.fn(async (_userId, month) => createRecap({ month, photoIds: [] })),
    saveSelection: vi.fn(async (selection) => createRecap({
      month: selection.month,
      photoIds: selection.selectedPhotoIds,
      selectionStatus: MonthlyRecapSelectionStatus.selected,
    })),
    skipSelection: vi.fn(async (_userId, month) => createRecap({
      month,
      photoIds: [],
...
function createPhotos(month: string, count: number): DailyPhoto[] {
  return Array.from({ length: count }, (_, index) => createPhoto(month, index + 1));
}
...
function createRecap({
  ...
  return {
    ...
    selectionStatus,
    promptedAt: null,
    completedAt: selectionStatus === MonthlyRecapSelectionStatus.selected ? `${month}-30T00:00:00.000Z` : null,
